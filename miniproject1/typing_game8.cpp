...
vector<string> loadSentences(const string& filename) {
    ...
    ifstream file(filename);  // 주어진 파일을 엽니다.
    if (!file) {  // 파일을 열 수 없으면
...
void startNewGame(RenderWindow& window, const vector<string>& sentences) {
    ...
    Clock cursorClock;
    bool showCursor = true;  // 커서 깜빡임 상태
            ...
            if (Keyboard::isKeyPressed(Keyboard::Key::Enter)) {
                if (userInput == currentSentence) {  // 문장이 정확히 일치하면
                    ...
                    if (sentenceIndex + 1 >= sentences.size()) {
                        ...
                        Text resultDisplay(font, resultText, 24);
                        ...
                        while (resultWindow.isOpen()) {
                            while (const optional<Event> resultEvent = resultWindow.pollEvent()) {
                                ...
                                if (resultEvent->is<Event::MouseButtonPressed>()) {
                        ...
                        sentenceText.setString(currentSentence);
                        userInput = "";
...
int main() {
    try {
        ...
        cerr << "예외 발생: " << e.what() << endl;  // 예외 처리
        return 1;
    }
    catch (...) {
        cerr << "알 수 없는 오류가 발생했습니다." << endl;  // 알 수 없는 오류 처리
        return 1;
    }
    return 0;
}
